...
def refine_model(doc, prompt, model_name):
    ...
    refine_chain = load_summarize_chain(
        llm,
        chain_type="refine",
        question_prompt=complete_prompt,
    ...
    with get_openai_callback() as cb:
        ...
        refine_outputs = refine_chain({"input_documents": [doc]})
        end_time = time.time()  # End timing
        execution_time = end_time - start_time  # Calculate total execution time
    return refine_outputs["output_text"], execution_time, cb.total_cost
